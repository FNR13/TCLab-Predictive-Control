...
addpath(fullfile(script_dir, '..', 'Common'));   % tclab.m lives there
try
    tclab;
    run = true;
catch
...
number_of_steps = size(u_values, 2);
T = step_duration * 2 + (number_of_steps - 1) * step_duration; % experiment duration [s] 
% First step has double the time 

Ts = 5; % sampling period [s]
N = T/Ts; % number of samples to collect (bc zero)

step_duration_samples = step_duration / Ts;

% Open-loop profile
u = zeros(2,N);
first_index = 2*step_duration_samples;
