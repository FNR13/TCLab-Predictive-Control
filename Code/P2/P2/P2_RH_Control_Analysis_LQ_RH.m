...
%% =========================================================
for p = 1:length(A_values)
    fig = figure('Name', sprintf('Controlo LQ — %s', plant_short{p}), ...
                 'Position', [170+p*30, 110, 1150, 680]);
    tiledlayout(2, 1, 'TileSpacing', 'compact', 'Padding', 'compact');

    % --- Top panel: LQ gain represented over the same horizon axis ---
    nexttile;
    hold on;
    h_lq_lines = gobjects(length(R_values), 1);
    for r_idx = 1:length(R_values)
        K_LQ_line = K_LQ_all(p, r_idx) * ones(size(H_values));
        h_lq_lines(r_idx) = plot(H_values, K_LQ_line, 'o-', ...
            'Color', colors(r_idx,:), 'LineWidth', 1.8, ...
            'MarkerSize', 5, 'MarkerFaceColor', colors(r_idx,:));
    end
    grid on; box on;
    xlabel('Horizonte H', 'FontSize', 11);
    ylabel('Ganho K_{LQ}', 'FontSize', 11);
    title(sprintf('Ganho LQ Representado no Mesmo Eixo de Horizonte — Planta %s', plant_short{p}), ...
          'FontSize', 12, 'FontWeight', 'bold');
    leg_labels_lq = cellfun(@(l) sprintf('K_{LQ} (%s)', l), R_labels, ...
                            'UniformOutput', false);
    legend(h_lq_lines, leg_labels_lq, 'Location', 'best', 'FontSize', 9.5, ...
           'NumColumns', 2);
    xlim([1 Hmax]);
    text(Hmax*0.98, min(ylim)*1.02 + range(ylim)*0.04, ...
         'LQ é horizonte infinito: valor constante em H', ...
         'HorizontalAlignment', 'right', 'FontSize', 8.5, ...
         'Color', [0.4 0.4 0.4]);

    nexttile;
    hold on;
    h_lq_eig = gobjects(length(R_values), 1);
    for r_idx = 1:length(R_values)
        eig_LQ_line = eig_LQ_all(p, r_idx) * ones(size(H_values));
        h_lq_eig(r_idx) = plot(H_values, eig_LQ_line, 's-', ...
            'Color', colors(r_idx,:), 'LineWidth', 1.8, ...
            'MarkerSize', 5, 'MarkerFaceColor', colors(r_idx,:));
    end
    ...
    grid on; box on;
    xlabel('Horizonte H', 'FontSize', 11);
    ylabel('|\lambda_{cl,LQ}|  =  |A - BK_{LQ}|', 'FontSize', 11);
    title(sprintf('Estabilidade LQ — Planta %s', plant_short{p}), ...
          'FontSize', 12, 'FontWeight', 'bold');
    leg_eig_labels_lq = cellfun(@(l) sprintf('|\lambda_{LQ}| (%s)', l), R_labels, ...
                                'UniformOutput', false);
    legend(h_lq_eig, leg_eig_labels_lq, 'Location', 'best', 'FontSize', 9.5, ...
           'NumColumns', 2);
    xlim([1 Hmax]);
    ...
    sgtitle(sprintf('Análise de Controlo LQ — %s', plant_short{p}), ...
            'FontSize', 13, 'FontWeight', 'bold');
end
...
%% =========================================================
for p = 1:length(A_values)
    fig = figure('Name', sprintf('Comparação RH vs LQ — %s', plant_short{p}), ...
                 'Position', [230+p*30, 140, 1150, 680]);
    tiledlayout(2, 1, 'TileSpacing', 'compact', 'Padding', 'compact');

  
    nexttile;
    hold on;
    h_comp = gobjects(length(R_values), 1);
    for r_idx = 1:length(R_values)
        K_RH = squeeze(K_RH_all(p, r_idx, :))';
        K_LQ = K_LQ_all(p, r_idx);
        h_comp(r_idx) = plot(H_values, K_RH, 'o-', ...
            'Color', colors(r_idx,:), 'LineWidth', 1.8, ...
            'MarkerSize', 5, 'MarkerFaceColor', colors(r_idx,:));
        yline(K_LQ, '--', 'Color', colors(r_idx,:), ...
    ...
    xlabel('Horizonte H', 'FontSize', 11);
    ylabel('Ganho K', 'FontSize', 11);
    title(sprintf('Comparação do Ganho: K_{RH}(H) vs K_{LQ} — Planta %s', plant_short{p}), ...
          'FontSize', 12, 'FontWeight', 'bold');
    legend(h_comp, leg_labels, 'Location', 'best', 'FontSize', 9.5, ...
           'NumColumns', 2);
    xlim([1 Hmax]);
    text(Hmax*0.98, min(ylim)*1.02 + range(ylim)*0.04, ...
         '-- K_{LQ} | linha contínua com marcadores = K_{RH}', ...
         'HorizontalAlignment', 'right', 'FontSize', 8.5, ...
         'Color', [0.4 0.4 0.4]);
    ...
    nexttile;
    hold on;
    h_comp_eig = gobjects(length(R_values), 1);
    for r_idx = 1:length(R_values)
        eig_RH = squeeze(eig_RH_all(p, r_idx, :))';
        eig_LQ = eig_LQ_all(p, r_idx);
        h_comp_eig(r_idx) = plot(H_values, eig_RH, 's-', ...
            'Color', colors(r_idx,:), 'LineWidth', 1.8, ...
            'MarkerSize', 5, 'MarkerFaceColor', colors(r_idx,:));
        yline(eig_LQ, '--', 'Color', colors(r_idx,:), ...
    ...
    grid on; box on;
    xlabel('Horizonte H', 'FontSize', 11);
    ylabel('|\lambda_{cl}|', 'FontSize', 11);
    title(sprintf('Comparação de Estabilidade: |\lambda_{RH}(H)| vs |\lambda_{LQ}| — Planta %s', plant_short{p}), ...
          'FontSize', 12, 'FontWeight', 'bold');
    legend(h_comp_eig, leg_eig_labels, 'Location', 'best', 'FontSize', 9.5, ...
           'NumColumns', 2);
    xlim([1 Hmax]);
    ...
    sgtitle(sprintf('Comparação Direta RH vs LQ — %s', plant_short{p}), ...
            'FontSize', 13, 'FontWeight', 'bold');
end

%% =========================================================
%% FIGURE 8 — Summary (A=1.2 )
%% =========================================================
A = 1.2;
...
fig8 = figure('Name', 'Horizonte Mínimo de Estabilização — A=1.2', ...
